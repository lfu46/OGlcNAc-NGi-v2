...
# protein level
WP_protein_sl_tmm_log2_Jurkat <- WP_protein_sl_tmm_Jurkat |> 
  mutate(
...
WP_protein_sl_tmm_log2_Data_Matrix_Jurkat <- data.matrix(WP_protein_sl_tmm_log2_Jurkat |> select(log2_Tuni_1_sl_tmm:log2_Ctrl_6_sl_tmm))
rownames(WP_protein_sl_tmm_log2_Data_Matrix_Jurkat) <- WP_protein_sl_tmm_log2_Jurkat$UniprotID

WP_protein_Fit_Jurkat <- lmFit(WP_protein_sl_tmm_log2_Data_Matrix_Jurkat, Experiment_Model)
...
write_csv(
  WP_protein_Top_tb_Jurkat,
  file = 'data_source/WP_differential_analysis/WP_protein_Top_tb_Jurkat.csv'
)
